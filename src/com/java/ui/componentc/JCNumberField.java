...
public class JCNumberField extends JCTextField implements Serializable {
    private class NumberDocument extends PlainDocument {
        ...
        public void insertString(int offset, String input, AttributeSet a)
            ...
            this.matcher = JCNumberField.this.pattern.matcher(this.text);

            if (((JCNumberField.this.maxLength > 0) && (this.length > JCNumberField.this.maxLength))
                    || (!this.matcher.matches())) {
                return;
            }

            try {
                double num = JCNumberField.this.getNumber(this.text.toString(),
                        Double.valueOf(-4.940656458412465E-324D)).doubleValue();

                if ((num > JCNumberField.this.maxRange)
                        || ((JCNumberField.this.nonNegative) && (num < 0.0D))) {
                    return;
                }
            } catch (Exception e) {
                return;
    ...
    private static final long serialVersionUID = -153560312096354219L;
    ...
    public JCNumberField() {
        this(0);
    }

    public JCNumberField(int maxLength) {
        this(maxLength, 0);
    }

    public JCNumberField(int maxLength, int decimalLength) {
        this(maxLength, decimalLength, 1.7976931348623157E+308D);
    }

    public JCNumberField(int maxLength, int decimalLength, double maxRange) {
